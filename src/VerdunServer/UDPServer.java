...
{
	VerdunOperations ver;
	public final int port = 5001;
	public UDPServer(VerdunOperations ver) {
		this.ver=ver;
	}
...

			{
				...
				try {
					...
					case "listMovieShowsAvailability":
						Gson gson1 = new Gson();
						if(VerdunOperations.movieList.containsKey(ctx)) {
							json = gson1.toJson(VerdunOperations.movieList.get(ctx));
						}
						break;
					case "getBookingSchedule":
						Gson gson = new Gson();
						if(VerdunOperations.customerMovieList.containsKey(ctx)) {
							json = gson.toJson(VerdunOperations.customerMovieList.get(ctx));
							System.out.println("Customer Movie List in Verdun: " + VerdunOperations.customerMovieList);
						}
						break;
					case "canBookInVerdun":
...
//						commonPOA ver = new VerdunOperations();
						json= ver.bookMovieTickets(customerID, movieName, movieID, noOfBooking);
						System.out.println("Customer Movie List in Verdun: " + VerdunOperations.customerMovieList);
